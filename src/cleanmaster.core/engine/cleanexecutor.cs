...
public sealed class CleanExecutor
{
    ...
    public CleanReport Execute(CleanRequest req, IProgress<CleanProgress>? progress, CancellationToken ct)
    {
        ...
        report.FreeBeforeBytes = SystemInfo.GetFreeBytes("C");
        ...
        if (req.CreateRestorePoint && !req.DryRun)
        {
            ...
            report.RestorePointResult = RestorePointHelper.Create($"{BuildInfo.ProductName} 清理前自动还原点");
        ...
        report.FreeAfterBytes = SystemInfo.GetFreeBytes("C");
        ...
        Action<string, long, long, long, long> tick)
    {
        ...
        {
            var (items, bytes) = RecycleBin.Query("C:\\");
            if (req.DryRun)
            {
                result.TrashedFiles = items;
                result.TrashedBytes = bytes;
            }
            else if (RecycleBin.Empty("C:\\", out var err))
            {
                result.TrashedFiles = items;
                result.TrashedBytes = bytes;
            }
        ...
        void HandleFile(WalkFile file)
        {
            ...
            switch (strategy)
            {
                ...
                case CleanStrategy.Trash:
                    if (PathUtil.IsLong(file.Path))
                    {
                        // 长路径回收站不支持：转入备份区（仍可恢复）
                        var sid = ensureSession();
                        ...
                        {
                            Skip(file, "长路径转入备份区失败：" + berr2);
                        }
                    }
                    else if (RecycleBin.TryRecycle(file.Path, out var terr))
                    {
                        result.TrashedFiles++;
                        result.TrashedBytes += file.Size;
                        tick(file.Path, 1, file.Size, 0, 0);
