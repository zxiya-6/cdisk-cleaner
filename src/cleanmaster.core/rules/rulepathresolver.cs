...
/// <summary>把规则中的路径模板（含 %ENV% 与通配段）解析为具体存在的路径集合。</summary>
public static class RulePathResolver
{
    ...
    public static ResolveResult Resolve(CategoryRule rule)
    {
        ...
        foreach (var raw in rule.Paths)
        {
            ...
            // UNC 或无法识别盘符的路径：不做通配展开
            if (expanded.StartsWith(@"\\", StringComparison.Ordinal) || !(expanded.Length >= 2 && expanded[1] == ':'))
            {
                if (PathUtil.HasWildcard(expanded))
                {
                    result.Warnings.Add($"暂不支持通配的 UNC 路径：{raw}");
                    continue;
                }

                AddIfExists(rule.Kind, expanded, result, dedupe);
                continue;
            }
        ...
        result.Paths.AddRange(dedupe.OrderBy(p => p, StringComparer.OrdinalIgnoreCase));
        return result;
    }

    private static void AddIfExists(RuleKind kind, string path, ResolveResult result, HashSet<string> dedupe)
    {
        var n = PathUtil.Normalize(path);
        if (n.Length == 0) return;
        if (kind == RuleKind.Files)
        {
            if (File.Exists(n)) dedupe.Add(n);
        }
    ...
    private static List<string> SplitSegments(string fullPath)
    {
        ...
        if (root.Length == 0) return segs;
        segs.Add(root.EndsWith('\\') ? root : root + "\\");
