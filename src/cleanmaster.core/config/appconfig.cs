...
{
    /// <summary>备份区根目录（默认 %LOCALAPPDATA%\C盘清理助手\backup）。</summary>
...
/// <summary>运行时路径（数据目录、日志、备份区、规则库定位）。</summary>
public static class AppPaths
{
    ...
    public static string DataRoot => Environment.GetEnvironmentVariable("CC5_DATA_DIR") is { Length: > 0 } over
        ...
        : Portable
            ...
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "C盘清理助手");
