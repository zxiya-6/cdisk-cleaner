...
internal static class Program
{
    ...
    private static int CmdClean(Dictionary<string, List<string>> o)
    {
        ...
        if (req.CreateRestorePoint) Console.WriteLine("将尝试创建系统还原点…");
        ...
        Console.WriteLine($"C 盘可用：{HumanSize.Format(report.FreeBeforeBytes)} → {HumanSize.Format(report.FreeAfterBytes)}（Δ {HumanSize.Format(report.FreeAfterBytes - report.FreeBeforeBytes)}）");
    ...
    private static int CmdBackupRestore(Dictionary<string, List<string>> o)
    {
        ...
        var entry = LedgerFactory.FromRestore(sid, report, before, SystemInfo.GetFreeBytes("C"));
        LedgerStore.Append(entry);
    ...
    private static int CmdBackupPurge(Dictionary<string, List<string>> o)
    {
        ...
        if (F(o, "all"))
        {
            ...
            LedgerStore.Append(LedgerFactory.FromPurge(reportAll, before, SystemInfo.GetFreeBytes("C")));
        ...
        LedgerStore.Append(LedgerFactory.FromPurge(report, before, SystemInfo.GetFreeBytes("C")));
        return report.FailedFiles > 0 ? 1 : 0;
    }
    ...
    private static int CmdSelfTest(Dictionary<string, List<string>> o)
    {
        ...
        Console.WriteLine($"样本目录：{V(o, "samples") ?? "（默认 %TEMP%\\cc5-selftest）"}");
        Console.WriteLine();
...
命令：
  ...
  bigfiles        大文件扫描 [--root C:\] [--min-mb 100] [--top 100] [--json 输出]
