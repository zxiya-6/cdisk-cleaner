...
/// <summary>内置自检（Linux 版）：扫描精度、安全策略、备份恢复、删除语义、台账、取消能力。</summary>
public static class SelfTestRunner
{
    public static List<TestResult> Run(string? sampleRoot, Action<string>? log = null)
    {
        sampleRoot ??= Path.Combine(Path.GetTempPath(), "cleanmaster-selftest");
        ...
        try
        {
            ...
            One("样本扫描精度（含排除/符号链接）", () => TestScanAccuracy(sampleRoot));
            One("大文件扫描", () => TestBigFiles(sampleRoot));
            One("备份区移动与恢复往返", () => TestBackupRoundtrip(sampleRoot));
            One("删除行为（权限/长路径）", () => TestDeletes(sampleRoot));
            One("回收站往返", () => TestTrashRoundtrip(sampleRoot));
    ...
    private static string TestSafetyFilter()
    {
        ...
        var mustDeny = new[]
        {
            ...
            "/proc/cpuinfo",
            "/sys/kernel",
            Path.Combine(home, "Documents", "report.docx"),
            Path.Combine(home, "Desktop", "todo.txt"),
            Path.Combine(home, ".ssh", "id_rsa"),
            Path.Combine(home, ".gnupg", "secring.gpg"),
            Path.Combine(home, ".local", "share", "Trash", "files", "x"),
            Path.Combine(home, "..", "other", "secret"),
        };

        var mustAllow = new[]
        {
            Path.Combine(home, ".cache", "thumbnails", "normal", "a.png"),
            Path.Combine(home, ".npm", "_cacache", "x"),
            Path.Combine(home, "Downloads", "installer.deb"),
        ...
        return $"拒绝 {mustDeny.Length} 项 / 放行 {mustAllow.Length} 项，全部符合预期（用户 {Environment.UserName}）";
    }
    ...
    private static string TestScanAccuracy(string sampleRoot)
    {
        ...
        // 符号链接（应被跳过，不得重复计数）
        var link = Path.Combine(sub1, "loop");
        var linkOk = MakeSymlink(link, dir);
        ...
        if (linkOk && cat.SkippedReparse < 1) throw new InvalidOperationException("符号链接未被跳过");

        return $"150 个文件 / {HumanSize.Format(expected)} 精确匹配；(符号链接创建={linkOk}) 跳过符号链接 {cat.SkippedReparse} 个";
    }
    ...
    private static string TestBigFiles(string sampleRoot)
    {
        ...
        if (result.Items.Count != 1 || !result.Items[0].Path.EndsWith("huge.bin", StringComparison.Ordinal))
    ...
    private static string TestDeletes(string sampleRoot)
    {
        ...
        File.WriteAllText(deepFile, "deep");
        if (deepFile.Length <= 300) throw new InvalidOperationException("长路径样本未达标：" + deepFile.Length);

        if (!FileOps.TryDeleteFile(deepFile, out var e3) || File.Exists(deepFile))
            throw new InvalidOperationException("长路径文件删除失败：" + e3);

        FileOps.TryDeleteDirectoryRecursive(dir, out _);

        return $"只读删除 OK；无权限目录 {(ok ? "root 下放行（符合预期）" : "正确拒绝（原因：“" + lockErr + "”）")}；长路径({deepFile.Length} 字符)删除 OK";
    }

    private static string TestTrashRoundtrip(string sampleRoot)
    {
        var src = Path.Combine(sampleRoot, "trash-src");
        Directory.CreateDirectory(src);
        var data = new byte[4096];
        Random.Shared.NextBytes(data);
        var f = Path.Combine(src, "t.bin");
        File.WriteAllBytes(f, data);

        // 备份原回收站状态
        var filesDir = Trashes.FilesDir;
        var infoDir = Trashes.InfoDir;
        var hadFiles = Directory.Exists(filesDir);
        var hadInfo = Directory.Exists(infoDir);

        try
        {
            if (!Trashes.TryRecycle(f, out var terr))
                throw new InvalidOperationException("移入回收站失败：" + terr);
            if (File.Exists(f)) throw new InvalidOperationException("源文件未移走");
            if (!Directory.Exists(filesDir) || !Directory.EnumerateFileSystemEntries(filesDir).Any())
                throw new InvalidOperationException("回收站 files/ 为空");
            if (!Directory.Exists(infoDir) || !Directory.EnumerateFileSystemEntries(infoDir).Any())
                throw new InvalidOperationException("回收站 info/ 为空");

            var (items, bytes) = Trashes.Query();
            if (items < 1 || bytes < data.Length)
                throw new InvalidOperationException($"查询不符：items={items} bytes={bytes}");

            if (!Trashes.Empty(out var eerr))
                throw new InvalidOperationException("清空回收站失败：" + eerr);
            if (Directory.Exists(filesDir) && Directory.EnumerateFileSystemEntries(filesDir).Any())
                throw new InvalidOperationException("清空后 files/ 仍有内容");
        }
        finally
        {
            // 还原回收站原状（不污染用户真实回收站）
            try { FileOps.TryDeleteDirectoryRecursive(filesDir, out _); } catch { }
            try { FileOps.TryDeleteDirectoryRecursive(infoDir, out _); } catch { }
            if (hadFiles) { try { Directory.CreateDirectory(filesDir); } catch { } }
            if (hadInfo) { try { Directory.CreateDirectory(infoDir); } catch { } }
        }

        return "移入回收站（含 .trashinfo）→ 查询命中 → 清空，往返一致";
    }
    ...
    private static string TestCancel()
    {
        ...
        var sw = Stopwatch.StartNew();
        var task = Task.Run(() => BigFileScanner.Scan("/usr", 100L * 1024 * 1024, 10, null, cts.Token));
        Thread.Sleep(120);
        cts.Cancel();
    ...
    private static bool MakeSymlink(string linkPath, string target)
    {
        try
        {
